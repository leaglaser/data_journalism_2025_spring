...
baltimore_county <- read_csv("balcodata.csv" , col_names = FALSE) |>
   clean_names() |>
...
**A1.: July 14, 2022 and October 4, 2022 had the most overdose calls, there were 23. I feel like there probably wasn't a day without any overdose cases because the most is 23 and that's a lot and considering multiple days had 23 is interesting.**

```{r}
baltimore_county_count <- baltimore_county |>
  group_by(date) |>
  summarise(count = n()) |>
  arrange(desc(count))
...
**A2: Saturday was the most common day for an overdose call, with 638 calls and 16% of calls coming in on Saturday.**

```{r}
newer_baltimore_county <- baltimore_county_count |>
  mutate(weekday = wday(date, label = TRUE, abbr = TRUE)) |>
  group_by(weekday) |>
  summarise(day_total = sum(count)) |>
  mutate(weekday_pct = day_total/4112) |>
  arrange(weekday)
...
**A3: 4540 SILVER SPRING RD is the most common overdose location with 36. I might be less confident in the counts by location because the accuracy of an overdose happening at this location vs the call just coming from that location is iffy.**

```{r}
newest_baltimore_county <- baltimore_county |>
  group_by(location) |>
  summarise(count = n()) |>
  arrange(desc(count))
...
**A4: What demographic is calling with overdose calls in Baltimore County and why those specific locations.**
