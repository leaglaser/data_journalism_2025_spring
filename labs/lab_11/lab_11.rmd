...
# get all the list‑entries (the <a> tags under the press-releases list)
nodes <- page %>% html_elements("a")  

# extract the text
text <- nodes %>% html_text2()

# the text looks like "11/12/2025: Statement from …"
# so split on the first “:”
df <- tibble(raw = text) %>%
  # drop possible empty or non‑release entries
  filter(str_detect(raw, "^\\s*\\d{1,2}/\\d{1,2}/\\d{4}:")) %>%
  mutate(
    date = str_extract(raw, "^\\s*\\d{1,2}/\\d{1,2}/\\d{4}"),
    title = str_replace(raw, "^\\s*\\d{1,2}/\\d{1,2}/\\d{4}:\\s*", "")
  ) %>%
  mutate(
    date = mdy(date)
  ) %>%
...
dates <- pg |>
  html_elements(".article__date") |>
  html_text(trim = TRUE)

dates_new <- data.frame(date = dates)

titles <- pg |>
  html_elements(".article__title > a") |>
  html_text(trim = TRUE)

titles_new <- data.frame(title = titles)

urls <- pg |>
  html_elements(".article__title > a") |>
  html_attr("href") |>
  str_c("https://www.vanhollen.senate.gov")

urls_new <- data.frame(url = urls)

press_new <- bind_cols(dates_df, titles_df, urls_df)

press_newer <- press_new |>
