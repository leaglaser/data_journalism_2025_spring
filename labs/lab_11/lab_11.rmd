...
**A1: The most common sanction in 2024 were Summary Suspension Affirmed and Surrender of License each with 7.** 

```{r}
alerts <- read.csv("data/Disciplinary_Alerts_2024.csv") |>
...
**A2: There are 9 press releases that have the word "Police" in it.** 
...
page <- url |>
  ...
  as.data.frame() |>
  clean_names() |>
  rename(press_releases = rvest_html_text_html_elements_read_html_url_p_wixui_rich_text_text) |>
  separate(press_releases, c("date", "press_releases"), ":") |>
  filter(!is.na(press_releases))

press_releases <-page |>
  filter(str_detect(press_releases, "Police"))
...
**A3: I'm curious what month is most likely to be the month with the most legislation passed or the most press releases surrounding enacting laws, not necessarily just news but lawmaking? Based on this data, I'm seeing a couple against  Trump so I'm curious how many press releases this year, alone, were around taking down Trump. **
...
van_hollen <- bind_cols (van_dates, van_titles, van_url) |>
  rename (date = value...1) |>
