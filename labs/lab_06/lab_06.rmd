...
three_counties <-bind_rows(allegany_911, carroll_911, cecil_911)
...
ZCTA_demographic <-read_csv("data/maryland_zcta.csv")

baltimore_911 <- baltimore_911 |>
  clean_names() |>
  glimpse()

ZCTA_demographic <- ZCTA_demographic |>
  clean_names() |>
...
baltimore_binded <- baltimore_911 |> left_join(ZCTA_demographic, join_by(zip_code))
...
A2: Cecil county in August 2022 had the most calls with 55.

```{r}
three_counties_new <- three_counties |>
...
A3: A lot of young people a lot of older people interesting how 81% are under 18. 

```{r}
#baltimore_binded_new <- 
...
baltimore_binded |>
  ...
  mutate(per_capita = call_count/population * 1000) |>
...
A4: What zip code has the most white non hispanice population? 21222 is the answers, that is Dundalk.
